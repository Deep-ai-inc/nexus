...
use nexus_llm::{Message, MessageContent, StreamingChunk};
use regex::Regex;
use std::sync::Arc;
        ...
        if message.role == nexus_llm::MessageRole::User {
                ...
                MessageContent::Structured(blocks) => {
                    ...
                    for block in blocks {
                        match block {
                            ...
                            nexus_llm::ContentBlock::ToolResult { content, .. } => {
            ...
            match &message.content {
                ...
                MessageContent::Structured(blocks) => {
                    for block in blocks {
                        match block {
                            nexus_llm::ContentBlock::Thinking { thinking, .. } => {
                            ...
                            nexus_llm::ContentBlock::ToolUse {
                                id, name, input, ..
                            } => {
                            ...
                            nexus_llm::ContentBlock::ToolResult { .. } => {
                                // Tool results are typically not part of assistant messages
                            }
                            nexus_llm::ContentBlock::RedactedThinking { summary, .. } => {
                                ...
                                for item in summary {
                                    ...
                                    match item {
                                        nexus_llm::ReasoningSummaryItem::SummaryText { text } => {
