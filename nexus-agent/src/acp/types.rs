...
/// Convert prompt content blocks from ACP to internal nexus_llm::ContentBlock format
pub fn convert_prompt_to_content_blocks(
    prompt: Vec<acp::ContentBlock>,
    base_path: Option<&std::path::Path>,
) -> Vec<nexus_llm::ContentBlock> {
    prompt
        .into_iter()
        .filter_map(|block| match block {
            acp::ContentBlock::Text(text_content) => {
                Some(nexus_llm::ContentBlock::new_text(text_content.text))
            }

            acp::ContentBlock::Image(image_content) => Some(nexus_llm::ContentBlock::Image {
...
fn convert_embedded_resource_to_text(
    ...
    match embedded_resource.resource {
        acp::EmbeddedResourceResource::TextResourceContents(text_resource) => {
            ...
            Some(nexus_llm::ContentBlock::new_text(formatted_text))
        }
...
mod tests {
    ...
    fn prompt_conversion_handles_text_and_images() {
        ...
        match &blocks[0] {
            nexus_llm::ContentBlock::Text { text, .. } => assert_eq!(text, "hello"),
            other => panic!("expected text block, got {other:?}"),
        }
        match &blocks[1] {
            nexus_llm::ContentBlock::Image {
                media_type, data, ..
            } => {
    ...
    fn prompt_conversion_handles_embedded_resources_without_base_path() {
        ...
        match &blocks[0] {
            nexus_llm::ContentBlock::Text { text, .. } => {
