...
use anyhow::Result;
use nexus_executor::CommandExecutor;
use nexus_llm::{
...
impl Agent {
    ...
    fn build_current_metadata(&self) -> Option<ChatMetadata> {
        ...
        self.session_id.as_ref().map(|session_id| {
            ...
            let mut total_usage = nexus_llm::Usage::zero();
            let mut last_usage = nexus_llm::Usage::zero();
    ...
    fn save_plan_snapshot_to_last_assistant_message(&mut self) {
        ...
        for &node_id in self.active_path.iter().rev() {
            if let Some(node) = self.message_nodes.get(&node_id) {
                if node.message.role == nexus_llm::MessageRole::Assistant {
    ...
    pub async fn load_from_session_state(
        ...
        if let Some(model_config) = self.session_model_config.as_ref() {
            ...
            let model_hint = nexus_llm::provider_config::ConfigurationSystem::load()
                .ok()
        ...
        self.next_request_id = session_state.next_request_id.unwrap_or_else(|| {
            self.message_history
                .iter()
                .filter(|msg| matches!(msg.role, nexus_llm::MessageRole::Assistant))
                .count() as u64
                + 1
        });
    ...
    async fn extract_tool_requests_from_response(
        &mut self,
        llm_response: &nexus_llm::LLMResponse,
        request_counter: u64,
    ) -> Result<(Vec<ToolRequest>, LoopFlow, nexus_llm::LLMResponse)> {
        let parser = ParserRegistry::get(self.tool_syntax());
    ...
    async fn execute_spawn_agent_parallel(
        ...
        let dummy_command_executor = nexus_executor::DefaultCommandExecutor;
    ...
    ) -> Result<(nexus_llm::LLMResponse, u64)> {
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                if e.to_string().contains("Streaming cancelled by user") {
                    ...
                    return Ok((
                        nexus_llm::LLMResponse {
                            content: Vec::new(),
                            usage: nexus_llm::Usage::zero(),
    ...
    fn context_usage_ratio(&mut self) -> Result<Option<f32>> {
        ...
        let limit = if let Some(limit) = self.context_limit_override {
            ...
            let config_system = nexus_llm::provider_config::ConfigurationSystem::load()?;
            config_system
