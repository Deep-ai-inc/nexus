...
use anyhow::Result;
use nexus_executor::{CommandExecutor, DefaultCommandExecutor, SandboxedCommandExecutor};
use nexus_llm::Message;
use nexus_sandbox::{SandboxContext, SandboxPolicy};
...
impl DefaultSubAgentRunner {
    ...
    async fn build_agent(
        ...
        let llm_provider =
            nexus_llm::factory::create_llm_client_from_model(&self.model_name, None, false, None).await?;
...
fn extract_last_assistant_text(messages: &[Message]) -> Option<String> {
    messages
        .iter()
        .rev()
        .find(|m| matches!(m.role, nexus_llm::MessageRole::Assistant))
...
fn extract_text_from_message(message: &Message) -> String {
    match &message.content {
        nexus_llm::MessageContent::Text(text) => text.clone(),
