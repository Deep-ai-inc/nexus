...
use nexus_sandbox::SandboxContext;
...
impl SessionInstance {
    ...
    pub fn get_current_context_size(&self) -> u32 {
        ...
        for message in self.session.messages.iter().rev() {
            if matches!(message.role, nexus_llm::MessageRole::Assistant) {
    ...
    #[allow(dead_code)]
    pub fn calculate_total_usage(&self) -> nexus_llm::Usage {
        let mut total = nexus_llm::Usage::zero();
    ...
    fn get_last_usage(&self) -> nexus_llm::Usage {
        for message in self.session.messages.iter().rev() {
            if matches!(message.role, nexus_llm::MessageRole::Assistant) {
                if let Some(usage) = &message.usage {
                    return usage.clone();
    ...
    pub fn generate_session_connect_events(&self) -> Result<Vec<UiEvent>, anyhow::Error> {
        ...
        if !buffered_fragments.is_empty() {
            ...
            let incomplete_message = MessageData {
                role: nexus_llm::MessageRole::Assistant,
    ...
    pub fn convert_messages_to_ui_data_until(
        ...
        let message_iter: Vec<(Option<crate::persistence::NodeId>, &nexus_llm::Message)> =
        ...
        for (node_id, message) in message_iter {
            if message.is_compaction_summary {
                let summary = match &message.content {
                    nexus_llm::MessageContent::Text(text) => text.trim().to_string(),
                    nexus_llm::MessageContent::Structured(blocks) => blocks
                        .iter()
                        .filter_map(|block| match block {
                            nexus_llm::ContentBlock::Text { text, .. } => Some(text.as_str()),
                            nexus_llm::ContentBlock::Thinking { thinking, .. } => Some(thinking.as_str()),
                            _ => None,
                        })
                        .collect::<Vec<_>>()
                        .join("\n")
            ...
            if message.role == nexus_llm::MessageRole::User {
                match &message.content {
                    nexus_llm::MessageContent::Text(text) if text.trim().is_empty() => continue,
                    nexus_llm::MessageContent::Structured(blocks) => {
                        let has_tool_results = blocks
                            .iter()
                            .any(|block| matches!(block, nexus_llm::ContentBlock::ToolResult { .. }));
                        if has_tool_results {
                            continue;
                        }
                    }
            ...
            match processor.extract_fragments_from_message(message) {
                Ok(fragments) => {
                    let role = match message.role {
                        nexus_llm::MessageRole::User => MessageRole::User,
                        nexus_llm::MessageRole::Assistant => MessageRole::Assistant,
                    };
                    messages_data.push(MessageData {
                        role,
                        fragments,
