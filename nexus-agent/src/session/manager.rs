...
use nexus_executor::{CommandExecutor, SandboxedCommandExecutor};
...
impl SessionManager {
    ...
    pub fn add_user_message(
        &mut self,
        session_id: &str,
        content_blocks: Vec<nexus_llm::ContentBlock>,
    ...
    #[allow(dead_code)]
    pub fn get_total_session_usage(&self) -> nexus_llm::Usage {
        if let Some(session_id) = &self.active_session_id {
            if let Some(session_instance) = self.active_sessions.get(session_id) {
    ...
    #[allow(dead_code)]
    pub fn get_session_usage(&self, session_id: &str) -> Option<(u32, nexus_llm::Usage)> {
        if let Some(session_instance) = self.active_sessions.get(session_id) {
    ...
    pub fn queue_structured_user_message(
        ...
        let text_representation = content_blocks
            ...
            .filter_map(|block| match block {
                nexus_llm::ContentBlock::Text { text, .. } => Some(text.clone()),
                nexus_llm::ContentBlock::Image { media_type, .. } => {
