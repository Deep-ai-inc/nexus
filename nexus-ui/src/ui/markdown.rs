...
impl MarkdownRenderer {
    ...
    fn horizontal_rule(&mut self) {
        self.column = std::mem::take(&mut self.column).push(
            Row::new()
                ...
                .push(Column::new().height(Length::Fixed(1.0)).width(Length::Fill).background(theme::TEXT_MUTED))
        );
    }
    ...
    fn flush_list_item(&mut self) {
        ...
        let row = Row::new().spacing(0.0)
            .push(TextElement::new(&marker).color(theme::TEXT_MUTED).source(self.source_id))
            .push(flow);
    ...
    fn build_inline_flow(&self, content: Vec<InlineSpan>) -> FlowContainer<'static> {
        ...
        for span in content {
            ...
            for word in words {
                ...
                if span.is_code {
                    elem = elem.color(theme::TOOL_ACTION);
                } else if span.is_link {
                    elem = elem.color(theme::TEXT_PATH);
                } else if span.strikethrough {
                    elem = elem.color(theme::TEXT_MUTED);
                } else {
                    elem = elem.color(theme::TEXT_PRIMARY);
                }
    ...
    fn wrap_in_blockquote<'a>(&self, inner: Row<'a>) -> Row<'a> {
        ...
        for _ in 0..self.blockquote_level {
            bq_row = bq_row
                .push(Column::new().width(Length::Fixed(3.0)).height(Length::Fixed(16.0)).background(theme::TEXT_MUTED))
                .fixed_spacer(8.0);
        }
        bq_row.push(inner)
    }
