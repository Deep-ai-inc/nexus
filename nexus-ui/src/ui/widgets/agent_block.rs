...
use crate::data::agent_block::{AgentBlock, AgentBlockState, PermissionRequest, PendingUserQuestion};
use crate::ui::theme;
use crate::utils::ids;
use crate::ui::widgets::{ToolWidget, ToolMessage};
...
impl<'a> AgentBlockWidget<'a> {
    ...
    pub fn view(block: &AgentBlock, question_input: Option<&strata::TextInputState>) -> Column<'static> {
        ...
        let query_source = ids::agent_query(block_id);
        let query_badge = Row::new()
            .padding_custom(Padding::new(2.0, 8.0, 2.0, 8.0))
            .background(Color::rgba(1.0, 1.0, 1.0, 0.06))
            .corner_radius(4.0)
            .spacing(6.0)
            .push(TextElement::new(">").color(theme::TEXT_MUTED).source(query_source))
            .push(TextElement::new(&block.query).color(theme::TEXT_PRIMARY).source(query_source));
        content = content.push(query_badge);

        // Thinking section
        if !block.thinking.is_empty() {
            let thinking_toggle_id = ids::agent_thinking_toggle(block_id);
            ...
            if !block.thinking_collapsed {
                ...
                let thinking_source = ids::agent_thinking(block_id);
                ...
                for line in thinking_preview.lines() {
                    content = content.push(
                        Row::new()
                            .fixed_spacer(16.0)
                            .push(TextElement::new(line).color(theme::THINKING).source(thinking_source)),
                    );
                }
            }
        }
        ...
        for (i, tool) in block.tools.iter().enumerate() {
            let toggle_id = ids::agent_tool_toggle(block_id, i);
            let tool_source = ids::agent_tool(block_id, i);
            content = content.push(ToolWidget::view(tool, toggle_id, tool_source));
        }

        // Permission dialog
        if let Some(ref perm) = block.pending_permission {
            let perm_source = ids::agent_perm_text(block_id);
            content = content.push(build_permission_dialog(
                perm,
                ids::agent_perm_deny(block_id),
                ids::agent_perm_allow(block_id),
                ids::agent_perm_always(block_id),
                perm_source,
            ));
        }
        ...
        if let Some(ref question) = block.pending_question {
            let q_source = ids::agent_question_text(block_id);
        ...
        if !block.response.is_empty() {
            let response_source = ids::agent_response(block_id);
            content = content.push(
                Row::new()
                    .spacing(6.0)
                    .cross_align(CrossAxisAlignment::Start)
                    .push(TextElement::new("\u{25CF}").color(theme::TEXT_MUTED)) // ●
                    .push(crate::ui::markdown::render(&block.response, response_source)),
            );
        }

        // Status footer
        let stop_id = ids::agent_stop(block_id);
    ...
    pub fn on_click(block: &AgentBlock, click_id: SourceId) -> Option<AgentBlockMessage> {
        ...
        // Thinking toggle
        if click_id == ids::agent_thinking_toggle(block_id) {
            return Some(AgentBlockMessage::ToggleThinking);
        }

        // Stop button
        if click_id == ids::agent_stop(block_id) {
            return Some(AgentBlockMessage::Stop);
        }

        // Permission buttons
        if click_id == ids::agent_perm_deny(block_id) {
            return Some(AgentBlockMessage::PermissionDeny);
        }
        if click_id == ids::agent_perm_allow(block_id) {
            return Some(AgentBlockMessage::PermissionAllow);
        }
        if click_id == ids::agent_perm_always(block_id) {
            return Some(AgentBlockMessage::PermissionAlways);
        }

        // Question submit
        if click_id == ids::agent_question_submit(block_id) {
        ...
        if let Some(ref question) = block.pending_question {
            for (q_idx, q) in question.questions.iter().enumerate() {
                for (o_idx, _) in q.options.iter().enumerate() {
                    if click_id == ids::agent_question_option(block_id, q_idx, o_idx) {
...
fn build_permission_dialog(
    ...
    let code_block = Column::new()
        ...
        .corner_radius(4.0)
        .push(TextElement::new(&perm.action).color(theme::WARNING).source(source_id));
    ...
    let mut dialog = Column::new()
        ...
        .width(Length::Fill)
        .push(TextElement::new("\u{26A0} Permission Required").color(theme::WARNING).source(source_id))
        .push(TextElement::new(&perm.description).color(theme::TEXT_SECONDARY).source(source_id))
        .push(code_block);

    if let Some(ref dir) = perm.working_dir {
        dialog = dialog.push(TextElement::new(format!("in {}", dir)).color(theme::TEXT_MUTED).source(source_id));
    }
        ...
        Row::new()
            ...
            .push(
                ButtonElement::new(deny_id, "Deny")
                    .background(theme::BTN_DENY)
                    .corner_radius(4.0),
            )
            .push(
                ButtonElement::new(allow_id, "Allow Once")
                    .background(theme::BTN_ALLOW)
                    .corner_radius(4.0),
            )
            .push(
                ButtonElement::new(always_id, "Allow Always")
                    .background(theme::BTN_ALWAYS)
                    .corner_radius(4.0),
            ),
    );
...
fn build_question_dialog(
    ...
    let mut dialog = Column::new()
        ...
        .width(Length::Fill)
        .push(TextElement::new("\u{2753} Claude is asking:").color(theme::TOOL_ACTION).source(source_id));

    for (q_idx, q) in question.questions.iter().enumerate() {
        dialog = dialog.push(
            TextElement::new(&q.question).color(theme::TEXT_PRIMARY).source(source_id)
        );

        let mut row = Row::new().spacing(8.0);
        for (o_idx, opt) in q.options.iter().enumerate() {
            let id = ids::agent_question_option(block_id, q_idx, o_idx);
            row = row.push(
    ...
    if let Some(input) = question_input {
        ...
        dialog = dialog.push(
            TextElement::new(label).color(theme::TEXT_SECONDARY).source(source_id)
        );
        let submit_id = ids::agent_question_submit(block_id);
        dialog = dialog.push(
...
fn build_footer(block: &AgentBlock, stop_id: SourceId) -> Row<'static> {
    let (status_text, status_color) = match &block.state {
        AgentBlockState::Pending => ("Waiting...", theme::TEXT_MUTED),
        AgentBlockState::Streaming => ("Streaming...", theme::RUNNING),
        AgentBlockState::Thinking => ("Thinking...", theme::THINKING),
        AgentBlockState::Executing => ("Executing...", theme::RUNNING),
        AgentBlockState::Completed => ("Completed", theme::SUCCESS),
        AgentBlockState::Failed(err) => (err.as_str(), theme::ERROR),
        AgentBlockState::AwaitingPermission => ("Awaiting permission...", theme::WARNING),
        AgentBlockState::Interrupted => ("Interrupted", theme::TEXT_MUTED),
    };

    let is_running = block.is_running();
    let footer_source = ids::agent_footer(block.id);
    ...
    if let Some(ms) = block.duration_ms {
        ...
        footer = footer.push(TextElement::new(&duration).color(theme::TEXT_MUTED).source(footer_source));
    }

    if let Some(cost) = block.cost_usd {
        footer = footer.push(
            TextElement::new(&format!("${:.4}", cost)).color(theme::TEXT_MUTED).source(footer_source),
        );
    }
    ...
    if total_tokens > 0 {
        footer = footer.push(
            TextElement::new(&format!("\u{2193} {}", format_tokens(total_tokens)))
                .color(theme::TEXT_MUTED).source(footer_source),
        );
    }
